...
\begin{enumerate}[leftmargin=0em, label=\arabic*., itemsep=-5pt]
    \item 解答用紙に記入された文字が判読困難な場合, ~採点されないことがあります。
    \item この問題冊子は，~13ページあります。問題は9問あり, ~一部の問題には小問が含まれます。配点は各問題ごとに明記されています。
    \item 試験中に問題冊子の印刷不鮮明，ページの落丁・乱丁及び問題の不備等に気付いた場合は，手を高く挙げて監督者に知らせなさい。
    \item 解答は，解答用紙を自作しそこに記入しなさい。しかし，選択式の問題であっても以下のような解答欄を作る必要はありません。
\end{enumerate}
% 解答欄の例
\begin{table}[h]
    \centering
    ...
    \newcommand{\eggfill}[1]{\,\raisebox{-3 pt}{
        \begin{tikzpicture}[x = 1 pt, y = 1 pt, line width = 1.1pt, scale = 1.0]
        \fill[gray, opacity=0.8] (0,0) ellipse (4.5 and 6);
        \draw (0,0) ellipse (4.5 and 6);\draw (0.1,0.2) node{\small\usefont{T1}{qhv}{m}{n}{#1}};\end{tikzpicture}}\,
    }
    ...
    \begin{tabularx}{0.67\textwidth}{!{\vrule width 1.2pt} p{1.65cm}| X !{\vrule width 1.2pt}}
        ...
        \hline
        \makebox[\hsize][c]{10} & \iibf{0}\iibf{1}\iibf{2}\!\eggfill{3}\!\!\iibf{4}\iibf{5}\iibf{6}\iibf{7}\iibf{8}... \\  
        \specialrule{1.2pt}{0pt}{0pt}
    \end{tabularx}
\end{table}
% 注意事項続き
\begin{enumerate}[leftmargin=0em, label=\arabic*., resume, itemsep=-5pt]
    \item 記述式の解答については, ~表記のゆれは採点において考慮されず, ~解答の内容が適切に伝われば正答とみなされます。
...
このとき, ~式\;\:\scalebox{1.15}{\(n_d\:g'' atf\mathsmaller{-}\)}\;が表すアニメの名称を答えよ。
\hspace{0.6em}\counterbox[-0.15em]\\\haiten{10}\\
...
\begin{adjustwidth}{1em}{0em} 
    ...
    その勝敗を分けることとなったこの試験の最終問題として最も適当なものを, 
    次の\!\egg{1}\textasciitilde\!\egg{4}のうちから一つ選べ。\counterbox[-0.2em] \\[3.5em]
...
\begin{adjustwidth}{1em}{0em} 
    ...
    以下のA \textasciitilde\;Fに示す登場人物の本名に対応するコードネームとして最も適当なものを, 
    次の\!\egg{1}\textasciitilde\!\raisebox{1.5px}{\egg{c}}のうちから一つずつ選べ。
    必要ならば\;\hyperlink{page.13}{「キャラクター一覧」}\;を参考にしてもよい。
    \counterbox[-0.20em]\hspace{4.0em}\raisebox{-1.3px}{〜}\hspace{0.4em}\addtocounter{boxcounter}{4}\counterbox[-0.20em]\hspace{4.0em}\\[1.6em]
...
\end{adjustwidth}
    ...
    \begin{tabularx}{\textwidth}{X X X}  % 3列を均等にする
        \ibf{A} 赤羽業  & \ibf{B} 岡野ひなた  & \ibf{C} 潮田渚 \\[0.8em]
        \ibf{D} 寺坂竜馬  & \ibf{E} 吉田大成  & \ibf{F} 殺せんせー \\
    \end{tabularx}
    \par
    \vspace{3.0em}
    \addtocounter{boxcounter}{-6}
    \hspace{-2.0em}\counterbox[-0.10em]\hspace{4.0em}\text{〜}\hspace{0.4em}\addtocounter{boxcounter}{4}\counterbox[-0.10em]\hspace{4.0em}
    \text{の解答群}
    \vspace{0.8em}
    \begin{center}
        \hspace{-2.0em}
        \makebox[\textwidth]{
            ...
            \begin{tabular}{|c l c l c l|}
                ...
                \iibf{7} & 性別  & \iibf{8} & ホームベース  & \iibf{9} & ツンデレスナイパー \\
                \iibf{a} & E組の闇  & \iibf{b} & 変態終末期  & \iibf{c} & 永遠なる疾風の運命の皇子 \\ 
                \hline
            \end{tabular}
        }
    \end{center}
...
% ---------------------------------------------------------3---------------------------------
\nextmondai
以下のグラフおよび図が示す内容から推測されるアニメとして最も適当なものを答えよ。\hspace{0.6em}\counterbox\hspace{3.8em}\haiten{10}\\[-0.1em]
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[draw=none] at ($(WW2.east) + (1.1,0.4)$) {\Large $\beta$};
...
\begin{center}
    \begin{tikzpicture}
        ...
        \node[draw=none, fill=beige] at (2.6, -0.2) {\large 小林カンナ};
        \node[draw=none, fill=beige] at (2.6, -1.2) {\large 暁美ほむら};
        \node[draw=none, fill=beige] at (2.4, -2.2) {\large 蛇喰夢子};
        % AとBの要素
        \node[draw=none, fill=beige] at (-1.0, 1.0) {\large 中野一花};
...
% \newcommand{\ibf}[1]{\text{#1}\hspace{0.5em}}
\begin{tabularx}{\textwidth}{X X X}  % 3列を均等にする
    \ibf{I} 椎名真昼  & \ibf{II} 西森柚咲 & \ibf{III} 楪いのり \\[0.8em]
\end{tabularx}
\par
\vspace{2em}
\addtocounter{boxcounter}{-3}
...
として最も適当なものを次の\!\egg{1}\textasciitilde\!\egg{3}のうちから一つずつ選べ。\haiten{10}\\
    ...
    \begin{adjustbox}{scale=0.43, center}
        \subfloat[]{
            \begin{tikzpicture}
                ...
                \node[anchor=north west, xshift=-2pt, yshift=10pt] at (img.north west) {\huge\huge\textbf{I}};
            \end{tikzpicture}
        }
        \hspace*{1em}
        \subfloat[]{
...
% -------------------------------------------------------------7-----------------------------
\nextmondai 
以下に示す文章は, ~あるアニメに関する口コミである。
この文章の内容に該当するアニメを推測し, ~その名称を答えよ。\counterbox[-0.1em] \hspace{3.5em} \haiten{10}
\vspace*{-1.1em}
\begin{center}
    \begin{tikzpicture}
        % コメント 1
...
ヤニカス教師が不快で即切った
１話の初っぱなから職員室でタバコを吸いまくる教師が出て来て不快極まりなくてすぐ観るのをやめました！
受動喫煙を助長させるような描写のアニメは非常に不快で有害で危険です！
厚労省の低い見積りでも、受動喫煙で年間１５０００人も殺されているのですから。
...
主人公にイライラ
主人公のどこか悟ってるような語り口調に耐えられず途中で視聴を断念。
ストーリーはベッタベタでありがちなストーリー。
女キャラ達は可愛いね。
主人公がひきこもりなので各所で「あー俺と似てるわー」みたいな
書き込みを見かけると失笑しか出ない。
\fi
\clearpage 
% ----------------------------------------------------------8--------------------------------
...
問1\:\textasciitilde\:問3の説明を読み,~
それぞれの説明が意味する単語を英語で答えよ。\haiten{12} \\[0.8em]
\begin{adjustwidth}{1em}{0em} 
    \raggedright
    \noindent\textbf{問\hspace{0.5em}1} \hspace{0.5em}
    \begingroup
        \spaceskip=0.4em
        A Japanese genre of science or fantasy fiction featuring a protagonist who is transported to or reincarnated in a different,
    ...
    \begingroup
        \spaceskip=0.4em
        A genre in Japanese anime and manga focuses on romantic or deeply emotional relationships
    ...
    \endgroup
    \vspace{2.6em}
    \noindent\textbf{問\hspace{0.5em}3} \hspace{0.5em}
    \begingroup
        \spaceskip=0.4em
        A type of self-published work, often in manga or novel format, created by independent artists or groups. 
